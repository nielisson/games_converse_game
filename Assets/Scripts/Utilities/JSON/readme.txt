...
JSONObject j = new(JSONObject.Type.OBJECT);
//number
...
<syntaxhighlight lang="csharp">
DoRequest(URL, new(delegate(JSONObject request) {
	request.AddField("sort", delegate(JSONObject sort) {
		sort.AddField("_timestamp", "desc");
	});
	request.AddField("query", new(delegate(JSONObject query) {
...
JSONObject j = new(encodedString);
accessData(j);
